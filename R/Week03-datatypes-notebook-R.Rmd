...
*Note* reminder of the namespace syntax which uses double colon `::` means `package::function()`. So the above on line 96 means the `glimpse()` function from the `{dplyr}` package 
...
If you run the following code chunk, it will stop execution at line 131 due to the error. Run each line of code line-by-line instead to see each line of code executed one at a time (131, 133, 135). 
...
```{r rounding-behavior}
# Based on what is mentioned above in line 290, what do you expect the output to be for the following round functions? Think about this before running the code line-by-line
...
Run code the code chunks on lines 433 (numeric-coercion-5) and 442 (numeric-coercion-6) a few times to see that indeed `sample()` randomly generates numbers, so the intergers in the vectors change in value each time. 
...
## note if you do not run the whole code chunk, you need to highlight all of the string2 object to run it. This means lines 564 to 566. Play around with this to see what I mean - e.g., put your cursor on line 565 and run the code to see what happens 
...
# un-comment the code above on line 580 and run it 
...
As described above in the Numeric section (Section 3), when converting a character object to numeric with `as.numeric()`, anything that cannot be parsed will turn into NA. This means anything other than numbers in quotation marks. 
...
c_vec # and of the character vector 
```

Logical vector to nominal and ordinal factor vectors 
...
Numeric vector to nominal and ordinal factor vectors 
...
Character vector to nominal and ordinal factor vectors 
...
Note that `monthsm_f` is not ordered, rather we have just sorted the factor levels in a useful way. To make the factor ordinal (AKA ordered), we can pass the data object to `as.ordered()` in our tidy data flow.   
...
In plain language, this code says "take the data object monthm and then make it a factor with the levels equal to months_levels, and then take that and make it an ordered factor."
...
```{r fct-coercion-6}
monthsm_f2 <- fct(c("feb", "Aug", "May", "Jun"), levels = months_levels) # this will produce an error as feb does not exist in month_levels
...
```{r fct-coercion-fix}
monthsm_f2 <- fct(c("Feb", "Aug", "May", "Jun"), levels = months_levels)
...
fct(c("Dec", "Apr", "Jan", "Mar"), levels = unique(monthsm))
...
You can change this so that NAs are stored in the levels, though at the cost of `is.na()` reporting accuracy. However, putting `NA`s in levels allows you to control its display. This is usually something I will do when creating tables or data visualizations where I want to show `NA` as a level of the factor. 
...
```{r timezones-1}
x1 <- ymd_hms("2026-06-01 12:00:00", tz = "America/New_York")
x1

x2 <- ymd_hms("2026-06-01 18:00:00", tz = "Europe/Copenhagen")
x2

x3 <- ymd_hms("2026-06-02 04:00:00", tz = "Pacific/Auckland")
x3
